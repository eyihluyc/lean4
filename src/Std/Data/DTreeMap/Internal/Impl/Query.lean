...
@[specialize] def toList : Impl α β → List ((a : α) × β a) :=
  go []
where
  go (acc : List ((a : α) × β a)) : Impl α β → List ((a : α) × β a)
  | .leaf => acc
  | .inner _ k v l r =>
    go (⟨k, v⟩ :: (go acc r)) l
...
/-- Returns a `List` of the key/value pairs in order. -/
@[specialize] def toList : Impl α (fun _ => β) → List (α × β) :=
  go []
where
  go (acc : List (α × β)) : Impl α (fun _ => β) → List (α × β)
  | .leaf => acc
  | .inner _ k v l r =>
    go (⟨k, v⟩ :: (go acc r)) l
